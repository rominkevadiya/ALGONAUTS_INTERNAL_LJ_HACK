...
- **`app/config.py`**: Central configuration storing model path (`model/best_resnet50_cifake.pth`), ImageNet normalization mean/std, spatial input dimensions (`224, 224`), class mapping (`0: FAKE`, `1: REAL`), and benchmark metrics constants.
...
graph TD
    ...
    Loader --> Checkpoint["Checkpoint File (model/best_resnet50_cifake.pth)"]
