...
4. [`04_training_and_evaluation.md`](04_training_and_evaluation.md) — Training hyperparameters (5 epochs, AdamW + CosineAnnealingLR), verified benchmark test metrics (97.42% accuracy, 0.9741 F1, 0.9964 ROC-AUC), corrected confusion matrix (9,786 TN / 9,787 TP), and metric formulas.
...
> [!NOTE]
> *Benchmark Note: All performance numbers reported in this documentation are benchmark results from the active **native32 checkpoint** (`model/best_resnet50_cifake_native32.pth`) evaluated on the official CIFAKE test dataset (20,000 images): **97.42% accuracy**, **0.9741 Macro F1**, **0.9964 ROC-AUC**. These numbers reflect the CIFAR-10 derived image distribution ($32 \times 32$ upscaled to $224 \times 224$). High-resolution digital photographs or modern webcam photos may exhibit domain shift.*
