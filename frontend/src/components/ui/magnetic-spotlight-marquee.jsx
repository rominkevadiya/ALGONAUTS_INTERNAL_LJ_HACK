...
export function MagneticSpotlightMarquee({
    ...
    const ctx = gsap.context(() => {
      const timer = setTimeout(() => {
        gsap.to(marqueeTrack, {
          x: `-${oneSetWidth}px`,
          duration: oneSetWidth / 80, // Calmed, smooth, non-nauseating speed
          ease: "none",
          repeat: -1,
          modifiers: {
  ...
  useEffect(() => {
    ...
    const measureGeometry = () => {
      ...
      if (!hasPointerMoved) {
        const titleEl = spotlightSection.querySelector('h1');
        const titleCenter = titleEl
          ? (titleEl.offsetTop + titleEl.offsetHeight * 0.45)
          : config.stripEdgeInset;
        const restY = Math.max(20, titleCenter - stripHeight / 2);
        stripTargetY = restY;
        stripCurrentY = restY;
        stripPrevY = restY;
    ...
    const initTimer = setTimeout(measureGeometry, 100);
    ...
    const handlePointerMove = (e) => {
      ...
      // Find the title element ("SignalScope")
      // Clamp follower so the strip center NEVER goes below the name SignalScope
      const titleEl = spotlightSection.querySelector('h1');
      const titleBottom = titleEl
        ? (titleEl.offsetTop + titleEl.offsetHeight * 0.8)
        : 240;

      const minTargetY = 20;
      const maxTargetY = Math.max(minTargetY, titleBottom - stripHeight / 2);
      const rawTargetY = pointerY - stripHeight / 2;

      stripTargetY = Math.max(minTargetY, Math.min(rawTargetY, maxTargetY));
    };
    ...
    const handlePointerLeave = () => {
      ...
      const titleCenter = titleEl
        ? (titleEl.offsetTop + titleEl.offsetHeight * 0.45)
        : config.stripEdgeInset;
      stripTargetY = Math.max(20, titleCenter - stripHeight / 2);
    };
  ...
  return (
    <section
      ref={containerRef}
      className={cn(
        "spotlight relative w-full h-[100vh] min-h-[800px] overflow-hidden bg-[#05080c] text-white font-sans select-none",
        className
      )}
    ...
    >
      ...
      >
        ...
        >
          {clonedImages.map((img, idx) => (
            <div key={idx} className="w-[140px] h-[140px] md:w-[180px] md:h-[180px] shrink-0 rounded-[20px] overflow-hidden shadow-sm bg-neutral-900 border border-white/10">
              <img
                src={img}
                alt="Marquee item"
      ...
      >
        ...
        {children && (
          <div className="w-full max-w-7xl mx-auto mt-12 pointer-events-auto flex flex-wrap items-center justify-center gap-4 z-50">
            {children}
          </div>
        )}
