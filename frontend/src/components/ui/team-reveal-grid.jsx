...
export function TeamRevealGrid({
  ...
  return (
    <section
      className={cn(
        "@container relative h-full min-h-[620px] w-full overflow-x-hidden overflow-y-auto bg-transparent px-4 py-8 text-neutral-950 dark:bg-transparent dark:text-white sm:px-7 sm:py-10",
        className,
      )}
      {...props}
    >
      ...
      <div className="relative mx-auto w-full max-w-5xl">
              ...
              >
                ...
                >
                  <div
                    ...
                    className={cn(
                      "relative overflow-hidden rounded-[1.35rem] border bg-white p-1.5 shadow-[0_10px_35px_-24px_rgba(0,0,0,0.42)] transition-[border-color,box-shadow,transform] duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] motion-reduce:transition-none dark:bg-[#0a1118]",
                      active
        ...
        {/* Lower Row (Remaining Specialists) - Dynamically pushes down when upper row is selected */}
        {members.length > 3 && (
          <ul
            className={cn(
              "grid grid-cols-2 gap-x-3 gap-y-6 @min-[560px]:grid-cols-3 @min-[560px]:gap-x-5 @min-[560px]:gap-y-5",
              "transition-[margin] duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]",
              members.slice(0, 3).some((m) => m.id === resolvedActiveId)
                ? "mt-12 @min-[560px]:mt-16"
                : "mt-6 @min-[560px]:mt-6"
            )}
          >
